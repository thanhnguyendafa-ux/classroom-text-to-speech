...
test("loads one source snapshot atomically and resets drilldown", () => {
  const local = libraryReducer(createLibraryState(), { type: "snapshotLoaded", source: "local", folders: [{ id: "f1", name: "Folder", createdAt: 1, lessons: [lesson("l1", "Alpha", "f1")] }], uncategorized: [] });
...
test("filters folders and lessons as derived state without mutating snapshot", () => {
  const state = libraryReducer(createLibraryState(), { type: "snapshotLoaded", source: "local", folders: [{ id: "f1", name: "Travel", createdAt: 1, lessons: [lesson("l1", "Airport", "f1"), lesson("l2", "Food", "f1")] }], uncategorized: [lesson("l3", "Airport Extra")] });
