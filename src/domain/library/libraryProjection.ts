...
export function projectCloudLibrary(folders: CloudFolder[], lessons: CloudLesson[]): LibraryDisplaySnapshot { return { folders: folders.map((folder) => ({ id: folder.id, name: folder.name, lessons: lessons.filter((lesson) => lesson.folderId === folder.id).map(copyLesson) })), uncategorized: lessons.filter((lesson) => lesson.folderId === null).map(copyLesson) }; }
export function projectLocalLibrary(folders: SavedFolder[], uncategorized: SavedLesson[]): LibraryDisplaySnapshot { return { folders: folders.map((folder) => ({ id: folder.id, name: folder.name, lessons: folder.lessons.map(copyLesson) })), uncategorized: uncategorized.map(copyLesson) }; }
